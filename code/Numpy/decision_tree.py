...
class Node:
    def __init__(self, X: pd.DataFrame, y:pd.Series, clf, depth = 0):
        self.X = X
        self.y = y
        self.depth = depth
        self.left = None
        ...
        self.values =  [counter[label] for label in self.y_labels] # list: values of y_labels
    ...
    def gini_index(self, y1:pd.Series, y2:pd.Series) -> float:
        return (self.gini_impurity(y1) * y1.shape[0] + \
                self.gini_impurity(y2) * y2.shape[0]) \
...
class DecisionTreeClassifier:
    def __init__(self, max_depth=None, min_samples_split=4, purity_function='gini'):
        # hyperparameters
    ...
    def fit(self, X_train, y_train):
        ...
        self.root = Node(X_train, y_train, self, depth=0)
        ...
        while len(self.queue) != 0:
            ...
            if self.purity_function != 'gini':
                ...
                if X_left.shape[0] == 0 or X_right.shape[0] == 0:
                    ...
                    node.left = Node(X_left, y_left, self,depth=node.depth+1)
                    node.right = Node(X_right, y_right, self, depth=node.depth+1)
...
if __name__ == '__main__':
    print(os.path.join(os.getcwd(), "assets"))
